...
namespace System.Speech.Internal.SrgsCompiler
{
	internal sealed class Backend
	{
		...
		private CfgGrammar.CfgSerializedHeader BuildHeader(List<State> sortedStates, int cBasePath, ushort iSemanticGlobals, out int cArcs, out float[] pWeights)
		{
			...
			cfgSerializedHeader.GrammarOptions = _grammarOptions | ((_alphabet != 0) ? GrammarOptions.IpaPhoneme : GrammarOptions.KeyValuePairs);
			...
			num3 += (uint)(_scriptRefs.Count * Marshal.SizeOf(typeof(CfgScriptRef)));
			...
			num3 = (cfgSerializedHeader.ulTotalSerializedSize = num3 + (uint)(cfgSerializedHeader.cPDB * Marshal.SizeOf(typeof(byte))));
		...
		private void CheckLeftRecursion(List<State> states)
		{
			...
			{
				state.CheckLeftRecursion(out var _);
			}
		}
