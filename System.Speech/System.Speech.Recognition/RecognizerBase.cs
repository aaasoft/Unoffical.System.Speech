	...
	internal class RecognizerBase : IRecognizerInternal, IDisposable, ISpGrammarResourceLoader
	{
		...
		internal void SetInputToDefaultAudioDevice()
		{
			lock (SapiRecognizer)
			{
				...
				try
				{
					...
					spObjectTokenCategory.GetDefaultTokenId(out var ppszCoMemTokenId);
		...
		private void InternalEmulateRecognizeAsync(string phrase, SpeechEmulationCompareFlags flag, bool useReco2, RecognizedWordUnit[] wordUnits)
		{
			...
			if (useReco2 || _supportsSapi53)
			{
				...
				spPhrase = ((wordUnits != null) ? SPPHRASE.CreatePhraseFromWordUnits(wordUnits, RecognizerInfo.Culture, out memHandles, out var coMem) : SPPHRASE.CreatePhraseFromText(phrase.Trim(), RecognizerInfo.Culture, out memHandles, out coMem));
				try
				{
		...
		private void ProcessBookmarkEvent(SpeechEvent speechEvent)
		{
			...
			{
				if (((uint)(int)speechEvent.WParam & (true ? 1u : 0u)) != 0)
				{
					SapiContext.Resume();
				}
			}
		...
		private RecognitionResult CreateRecognitionResult(SpeechEvent speechEvent)
		{
			...
			spRecoResult.Serialize(out var ppCoMemSerializedResult);
			byte[] array = null;
			try
			{
