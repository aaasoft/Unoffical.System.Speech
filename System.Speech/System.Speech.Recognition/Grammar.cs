...
namespace System.Speech.Recognition
{
	...
	public class Grammar
	{
		...
		{
			stream.Position = 0L;
			if (CfgGrammar.LoadIL(stream, out var assemblyContent, out var assemblyDebugSymbols, out var scripts))
			{
		...
		private SrgsRule[] RunOnInit(bool stg)
		{
			...
			if (text != null)
			{
				if (_proxy != null)
				{
					result = _proxy.OnInit(text, _parameters, _onInitParameters, out var exceptionThrown);
					flag = true;
		...
		private static string CheckRuleName(Stream stream, string rulename, bool isImportedGrammar, bool stgInit, out bool sapi53Only, out GrammarOptions grammarOptions)
		{
			...
			using (StreamMarshaler streamHelper = new StreamMarshaler(stream))
			{
				...
				sapi53Only = (cfgHeader.GrammarOptions & (GrammarOptions.SemanticInterpretation | GrammarOptions.IpaPhoneme | GrammarOptions.STG)) != 0;
