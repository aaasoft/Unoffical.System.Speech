	...
	internal class SapiRecognizer : IDisposable
	{
		...
		internal RecognizerInfo GetRecognizerInfo()
		{
			...
			return (RecognizerInfo)_proxy.Invoke(delegate
			{
				...
				try
				{
					sapiObjectToken.GetId(out var ppszCoMemTokenId);
		...
		internal IntPtr GetFormat(SPSTREAMFORMATTYPE WaveFormatType)
		{
			...
			{
				_proxy.Recognizer.GetFormat(WaveFormatType, out var _, out var ppCoMemWFEX);
				return ppCoMemWFEX;
			});
		}
		...
		private static object GetProperty(ISpRecognizer sapiRecognizer, string name, bool integer)
		{
			...
			if (integer)
			{
				sAPIErrorCodes = (SAPIErrorCodes)sapiRecognizer.GetPropertyNum(name, out var plValue);
				obj = plValue;
			}
			else
			{
				sAPIErrorCodes = (SAPIErrorCodes)sapiRecognizer.GetPropertyString(name, out var ppCoMemValue);
				obj = ppCoMemValue;
			}
			if (sAPIErrorCodes == SAPIErrorCodes.S_FALSE)
			{
