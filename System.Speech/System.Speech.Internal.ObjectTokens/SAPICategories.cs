	...
	internal static class SAPICategories
	{
		...
		internal static int DefaultDeviceOut()
		{
			...
			using (ObjectTokenCategory objectTokenCategory = ObjectTokenCategory.Create("HKEY_CURRENT_USER\\SOFTWARE\\Microsoft\\Speech\\AudioOutput"))
			{
				...
				{
					if (objectTokenCategory.TryGetString("DefaultTokenId", out var value))
					{
		...
		private static ObjectToken GetPreference(string category, string defaultLocation)
		{
			...
			using (ObjectTokenCategory objectTokenCategory = ObjectTokenCategory.Create(category))
			{
				if (objectTokenCategory != null)
				{
					if (objectTokenCategory.TryGetString(defaultLocation, out var value))
					{
		...
		private static int CompareTokenVersions(ObjectToken token1, ObjectToken token2, out bool pfDidCompare)
		{
			...
			if (registryDataKey != null)
			{
				registryDataKey.TryGetString("Vendor", out var value);
				registryDataKey.TryGetString("ProductLine", out var value2);
				registryDataKey.TryGetString("Version", out var value3);
				registryDataKey.TryGetString("Language", out var value4);
				if (registryDataKey2 != null)
				{
					registryDataKey2.TryGetString("Vendor", out var value5);
					registryDataKey2.TryGetString("ProductLine", out var value6);
					registryDataKey2.TryGetString("Version", out var value7);
					registryDataKey2.TryGetString("Language", out var value8);
		...
		private static bool ParseVersion(string s, ushort[] Version)
		{
			...
			else
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					if (!ushort.TryParse(s2, out var result) || result > 9999)
					{
						flag = false;
						break;
					}
